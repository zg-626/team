...
class StoreOrderRepository extends BaseRepository
{
    ...
    public function addCommission(int $merId, $order)
    {
        ...
        // 当前商务的信息，只发放一次
        if(in_array($salesman['uid'],$userId)){
            $processedUids[] = 327699;
        }

        // 单独处理省市区代理商：根据商家所在省市区分配省市区代理商的佣金
        $processedUids[]=$this->distributeRegionalCommission($order['order_id'], $userRepository, $userBillRepository, $userGroupRepository, $merchant, $money, $processedUids);
        ...
        while ($salesman->superior_uid !== 0) {
            ...
            // 如果是大区经理，终止循环
            if ($superior && $superior['group_id'] == self::USER_GROUP['AREA_MANAGER']) {
     ...
     * @param UserBillRepository $userBillRepository 用户账单仓库
     * @param UserGroupRepository $userGroupRepository 用户组仓库
     * @param int $province_id 省份
     * @param int $city_id 城市
     * @param int $district_id 区县
     * @param float $money 佣金基数
     * @param array &$processedUids 已处理用户ID数组
     * @return array
     */
    protected function distributeRegionalCommission($orderId, $userRepository, $userBillRepository, $userGroupRepository, $merchant, $money, &$processedUids)
    {
        // 提取商家的省市区
        ...
        $merchantProvinceId = $merchant->province_id;
        $merchantCityId = $merchant->city_id;
        $merchantDistrictId = $merchant->district_id;

        // 判断特殊直辖市的情况
        /*if($merchantCity ==='市辖区'){
            $merchantCityId = $merchant->province_id;
        }*/

        // 查询省级代理用户
        //$provinceUser = $userRepository->getWhere(['province_id' => merchantProvinceId,'city_id' => 0,'district_id'=>0 'group_id' => self::USER_GROUP['AGENT_3']]);

        // 查询市级代理用户
        $cityUser = $userRepository->getWhere(['city_id' => $merchantCityId,'district_id'=>0, 'group_id' => self::USER_GROUP['AGENT_2']]);
        ...
        if ($districtUser && $cityUser) {
            ...
            if (!in_array($cityUser['uid'], $processedUids, true)) {
                ...
                $agent=$cityUser;
                Log::info('讲师'.$agent['lecturer']);
                // 给市级讲师发放佣金
                if (!empty($agent['lecturer'])) {
                    // 获取讲师信息
                    $lecturer = $userRepository->get($agent['lecturer']);
                    if ($lecturer && !in_array($lecturer['uid'], $processedUids)) {
                        // 获取讲师分组信息及比例
        ...
        // 处理省级代理佣金 TODO 客户没确定，暂时不给佣金
        /*if ($provinceUser && !in_array($provinceUser['uid'], $processedUids)) {
            $provinceGroup = $userGroupRepository->get($provinceUser['group_id']);
            $province_commission = bcmul($provinceGroup->extension/100, $money, 2);
            $this->giveBrokerage($orderId, $userBillRepository, $provinceUser, $province_commission, $this->getSuperiorTitle($provinceUser['group_id']));

            // 记录已处理的用户ID
            $processedUids[] = $provinceUser['uid'];

        }*/
    }

    /**
     * 处理代理商绑定的上级（大区经理和区域经理）
     *
     * @param string $orderId 订单ID
     * @param UserRepository $userRepository 用户仓库
     * @param UserBillRepository $userBillRepository 用户账单仓库
     * @param UserGroupRepository $userGroupRepository 用户组仓库
     * @param object $agent 代理商用户对象
     * @param float $money 佣金基数
     * @param array &$processedUids 已处理用户ID数组
     * @return array
     */
    protected function processAgentSuperiors($orderId, $userRepository, $userBillRepository, $userGroupRepository, $agent, $money, &$processedUids)
    {
        // 处理代理商的上级（区域经理，大区经理）
        $superior_uid = $agent->superior_uid;

        if ($superior_uid !== 0) {
            $superior = $userRepository->get($superior_uid);
            if ($superior && !in_array($superior['uid'], $processedUids)) {
                $superiorGroup = $userGroupRepository->get($superior['group_id']);
        ...
        while ($salesman->superior_uid !== 0) {
            ...
            if ($superior['group_id'] == self::USER_GROUP['AGENT_1']) {
                ...
                if ($superior->superior_uid !== 0) {
                    ...
                    // 查询省级信息
                    $province_agent = $userRepository->get($city_agent->superior_uid);
     ...
     * @day 2023/5/4
     */
    public function sendCdkey($order)
    {
        ...
        $rest = $productAttrValueRepository->getWhere($where,'*',['productCdkey' => function($query) {$query->where('status',1)->order('cdkey_id ASC')->withLimit(1);}]);
        $cdkey = $rest['productCdkey'][0];
        if ($cdkey['is_type']) {
            $msg = '卡号：'.$cdkey['key'];
            if ($cdkey['pwd']) $msg .= ';卡密：'.$cdkey['pwd'];
            $cdkey->status  = -1;
            $cdkey->save();
        } else {
            $msg = $cdkey['key'];
    ...
    public function computed(StoreOrder $order, User $user)
    {
        ...
        if ($order->extension_one > 0 && $spreadUid) {
            ...
            $userBillRepository->incBill($spreadUid, 'coupon_amount', 'order_one', [
                ...
                'status' => 0,
                'title' => '获得推广抵用券',
                'number' => round($order->handling_fee*0.1,2),// 手续费的10%
                'mark' => $user['nickname'] . '成功消费' . (float)$order['pay_price'] . '元,奖励推广抵用券' . (float)round($order->handling_fee*0.1,2),
                'balance' => 0
            ]);
            $userRepository = app()->make(UserRepository::class);
            $userRepository->incBrokerage($spreadUid, round($order->handling_fee*0.1,2));
    ...
    public function takeOrder($id, ?User $user = null)
    {
        ...
        if ($order['status'] != 1 || ($order['order_type'] && $order['order_type'] != 20))
...
//        }
    ...
    public function orderDumpInfo($id, $data, $merId)
    {
        ...
        $orderInfo = $this->dao->getWhere(['order_id' => $id]);
        ...
        foreach ($orderInfo->orderProduct as $item) {
            $cargo .= mb_substr($item['cart_info']['product']['store_name'],0,10). ' ' .$item['cart_info']['productAttr']['sku']  .' * ' .$item['product_num'].$item['cart_info']['product']['unit_name'].PHP_EOL;
        ...
        $expData['siid'] = merchantConfig($merId,'mer_config_siid');
    ...
    public function batchDelivery(int $merId, array $params)
    {
        ...
        foreach ($params['order_id'] as $item) {
            ...
            if (!$ret  || $ret['mer_id'] != $merId || $ret['is_del'] != 0 || $ret['paid'] != 1 || $ret['delivery_type'] || $ret['order_type'] == 1) {
                $imp['status'] = 0;
    ...
    public function cityDelivery(int $id, int $merId, array $data, $service_id)
    {
        ...
        // 小程序发货管理
        event('mini_order_shipping', ['product', $order, 5, '', '']);
    ...
    {
        $data = $this->dao->get($id);
        $form = Elm::createForm(Route::buildUrl('pointsOrderMark', ['id' => $id])->build());
        $form->setRule([
            Elm::text('remark', '备注：', $data['remark'])->placeholder('请输入备注')->required(),
        ]);
        return $form->setTitle('订单备注');
...
//        $countPay = bcadd($countPay1, $countPay2, 2);
    ...
    public function batchPrinter(int $id, int $merId)
    {
        ...
        $data = [
            ...
            'user_address' => $order['user_address'],
            'total_price'  => $order['total_price'],
            'coupon_price' => bcadd(bcadd($order['coupon_price'], $order['svip_discount'], 2), $order['integral_price'], 2),
            'pay_price'    => $order['pay_price'],
            'total_postage' => $order['total_postage'],
            'pay_postage'   => $order['pay_postage'],
            'mark' => $order['mark'],
